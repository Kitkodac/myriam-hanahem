...
if(bishopArt){
  bishopArt.style.background='linear-gradient(90deg,rgba(0,0,0,.22),rgba(0,0,0,.08)),url(https://live.staticflickr.com/8579/28601032925_2f7c575e32_o.jpg) 50% 28%/cover no-repeat';
}
if(bishopCard){
  bishopCard.href='https://www.imdb.com/title/tt38266508/';
  bishopCard.target='_blank';
  bishopCard.rel='noopener';
...
.process-steps>div{cursor:pointer;position:relative;transition:background .25s,color .25s,transform .25s}.process-steps>div:after{content:'VIEW →';position:absolute;left:12px;bottom:12px;font-size:7px;letter-spacing:.14em;opacity:0;transition:.25s}.process-steps>div:hover{background:#191816;color:#f2eee7;transform:translateY(-2px)}.process-steps>div:hover:after{opacity:1}.process-shell{max-width:760px;background:#f2eee7;color:#171513;padding:52px 58px 46px}.process-shell .process-close{color:#fff}.process-kicker{font-size:9px;letter-spacing:.18em;margin-bottom:18px}.process-number{font:500 66px/.8 "Cormorant Garamond",serif;margin-bottom:14px}.process-title{font:500 54px/.88 "Cormorant Garamond",serif;margin:0 0 22px}.process-copy{font:400 20px/1.4 "Cormorant Garamond",serif;margin:0}.process-hint{text-align:center;margin:20px 0 0;font-size:8px;letter-spacing:.15em;opacity:.55}
@media(max-width:900px){.preview-badge{top:12px;left:12px}.video-modal,.process-modal{padding:14px}.video-close,.process-close{top:-42px}.quebec-section{display:block}.quebec-image{height:220px}.quebec-copy{padding:44px 18px 52px}.quebec-copy h2{font-size:44px;line-height:.88}.quebec-copy>p:not(.eyebrow){font-size:17px}.qc-projects{grid-template-columns:1fr;gap:10px;margin-top:28px}.qc-card{min-height:300px}.qc-meta h3{font-size:30px}.process-section{padding:42px 18px}.process-section .section-title h2{font-size:40px}.process-steps div{min-height:96px;padding:15px 12px}.process-steps b{font-size:28px;margin-bottom:16px}.process-steps>div:after{opacity:.55}.process-shell{padding:38px 24px 32px}.process-number{font-size:54px}.process-title{font-size:42px}.process-copy{font-size:18px}}
`;
...
// Interactive costume-design process
const processContent={
  en:[
    ['SCRIPT','Reading character arcs, period, social context and production needs to establish the costume language before any sourcing begins.'],
    ['RESEARCH','Building a visual world through period references, social codes, photography, archives, real people and location-specific detail.'],
    ['PALETTE & TEXTURE','Defining colour, fabric, wear, silhouette and texture so every costume supports story, hierarchy and emotional tone.'],
    ['FITTINGS','Testing proportion, movement, continuity and actor comfort while refining the visual identity of each character.'],
    ['CHARACTER','Turning research and fitting choices into a coherent wardrobe that feels lived-in, specific and truthful to the person on screen.'],
    ['SCREEN','Maintaining continuity and visual consistency through production so the costume work serves performance, camera and final storytelling.']
  ],
  fr:[
    ['SCÉNARIO','Lecture des arcs des personnages, de l’époque, du contexte social et des besoins de production afin de définir le langage costume avant les recherches et achats.'],
    ['RECHERCHE','Construction d’un univers visuel à partir d’archives, de références d’époque, de codes sociaux, de photographies et de détails propres aux lieux.'],
    ['PALETTE & MATIÈRES','Définition des couleurs, tissus, usure, silhouettes et textures afin que chaque costume soutienne le récit, la hiérarchie et le ton émotionnel.'],
    ['ESSAYAGES','Validation des proportions, du mouvement, de la continuité et du confort des interprètes tout en affinant l’identité visuelle de chaque personnage.'],
    ['PERSONNAGE','Transformation des recherches et des essayages en une garde-robe cohérente, crédible, précise et profondément liée au personnage.'],
    ['ÉCRAN','Suivi de la continuité et de la cohérence visuelle pendant le tournage afin que le costume serve le jeu, la caméra et le récit final.']
  ]
};
const pmodal=document.createElement('div');
pmodal.className='process-modal';
pmodal.setAttribute('aria-hidden','true');
pmodal.innerHTML='<div class="process-shell"><button class="process-close" aria-label="Close">×</button><div class="process-kicker">COSTUME DESIGN PROCESS</div><div class="process-number"></div><h3 class="process-title"></h3><p class="process-copy"></p><div class="process-hint">CLICK OUTSIDE OR × TO CLOSE</div></div>';
...
pmodal.querySelector('.process-close').addEventListener('click',closeProcess);
pmodal.addEventListener('click',e=>{if(e.target===pmodal)closeProcess()});
document.addEventListener('keydown',e=>{if(e.key==='Escape')closeProcess()});
document.querySelectorAll('.process-steps>div').forEach((step,i)=>{
  step.setAttribute('role','button');step.setAttribute('tabindex','0');step.setAttribute('aria-label',`Open process step ${i+1}`);
  const open=()=>{const l=root.dataset.lang==='fr'?'fr':'en';const item=processContent[l][i];pmodal.querySelector('.process-kicker').textContent=l==='fr'?'PROCESSUS DE CRÉATION COSTUME':'COSTUME DESIGN PROCESS';pmodal.querySelector('.process-number').textContent=String(i+1).padStart(2,'0');pmodal.querySelector('.process-title').textContent=item[0];pmodal.querySelector('.process-copy').textContent=item[1];pmodal.querySelector('.process-hint').textContent=l==='fr'?'CLIQUER À L’EXTÉRIEUR OU × POUR FERMER':'CLICK OUTSIDE OR × TO CLOSE';pmodal.classList.add('open');pmodal.setAttribute('aria-hidden','false');document.body.style.overflow='hidden';};
  step.addEventListener('click',open);step.addEventListener('keydown',e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();open();}});
});